# apply-eco-systems.ps1
# Adds: Food screen, Vet system, Travel/Expeditions, Greenhouse, Nav redesign
...
$patchFile = Join-Path $projectRoot "eco-patch-systems.cjs"

Write-Host ""
Write-Host "=== APPLY ECO SYSTEMS ===" -ForegroundColor Cyan
...
const MATERIAL_ICONS = { wood:'🪵', stone:'🪨', glass:'🪟', brick:'🧱' };
...
SR(
  `/* HOME SCREEN */`,
  `/* ============================================================
   FOOD SCREEN — feed animals, use vet packs
   ============================================================ */
  ...
  return (
    <div style={{padding:'24px 28px 40px'}}>
      <div style={{marginBottom:18}}>
        ...
        <div style={{fontSize:12,color:'#a8a29e',marginTop:4}}>Feed your animals to keep them earning · heal injuries with vet packs</div>
      </div>

      {/* Tabs */}
      <div style={{display:'flex',gap:8,marginBottom:20}}>
        {[['feed','🍎 FEED'],['heal','🏥 HEAL'],['shop','🛒 BUY FOOD']].map(([id,label])=>(
          ...
          <div>
                ...
                return (
                  <button key={food.id} onClick={()=>qty>0&&setSelFood(food.id)} disabled={qty===0} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,border:isSel?'1px solid #4ade80':'1px solid rgba(255,255,255,0.08)',background:isSel?'rgba(74,222,128,0.12)':'rgba(255,255,255,0.03)',cursor:qty>0?'pointer':'not-allowed',opacity:qty===0?0.4:1,textAlign:'left'}}>
                    ...
                    <div style={{textAlign:'right',flexShrink:0}}>
                      <div style={{fontSize:10,color:'#4ade80',fontFamily:'"JetBrains Mono",monospace',fontWeight:700}}>×{qty}</div>
                    </div>
                  </button>
                );
              })}
            ...
            {selCard&&selFood&&(
              <button onClick={handleFeed} style={{width:'100%',marginTop:12,padding:'13px',borderRadius:10,background:'linear-gradient(135deg,#22c55e,#16a34a)',border:'none',color:'#fff7ed',cursor:'pointer',fontFamily:'"Bebas Neue",sans-serif',fontSize:15,letterSpacing:'0.1em',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
                🍽️ FEED {selCard.first} {selCard.last}
              </button>
            )}
          </div>
        </div>
      ...
      {tab==='heal' && (
        <div>
          {injuredCards.length===0 ? (
            <div style={{padding:'40px',textAlign:'center',color:'#78716c',fontFamily:'"Outfit",sans-serif',fontSize:14}}>No injured animals right now. 🎉</div>
          ) : (
                  ...
                  <div key={card.id} style={{display:'flex',alignItems:'center',gap:14,padding:'14px 16px',borderRadius:12,background:'rgba(255,255,255,0.03)',border:`1px solid ${info.color}44`}}>
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:6,marginTop:4}}>
                        ...
                        <span style={{fontSize:9,color:'#78716c',fontFamily:'"JetBrains Mono",monospace'}}>needs: {info.healLabel} (×{inv[packKey]||0} in bag)</span>
                      </div>
                    </div>
      ...
      {tab==='shop' && (
        <div>
              ...
              return (
                <button key={food.id} onClick={()=>canAfford&&onBuyFood(food.id,food.price)} disabled={!canAfford} style={{padding:'12px 14px',borderRadius:12,background:canAfford?'rgba(255,255,255,0.04)':'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.08)',cursor:canAfford?'pointer':'not-allowed',opacity:canAfford?1:0.5,textAlign:'left',display:'flex',alignItems:'center',gap:10}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:10,color:'#fb923c',fontFamily:'"JetBrains Mono",monospace',fontWeight:700,marginTop:4}}>{food.price.toLocaleString()} ECO · ×{inv[food.id]||0} owned</div>
                  </div>
                </button>
              );
            })}
          ...
          {/* Vet packs shop */}
          <div style={{fontFamily:'"Bebas Neue",sans-serif',fontSize:16,color:'#fff7ed',letterSpacing:'0.1em',marginBottom:10}}>🏥 VET PACKS</div>
            ...
            {Object.entries(INJURY_LEVELS).map(([level,info])=>{
              ...
              return (
                <button key={level} onClick={()=>canAfford&&onBuyFood(packKey,info.vetPackPrice)} disabled={!canAfford} style={{padding:'12px 14px',borderRadius:12,background:canAfford?`${info.color}12`:'rgba(255,255,255,0.02)',border:`1px solid ${info.color}${canAfford?'44':'22'}`,cursor:canAfford?'pointer':'not-allowed',opacity:canAfford?1:0.5,textAlign:'left'}}>
                  ...
                  <div style={{fontSize:10,color:'#fb923c',fontFamily:'"JetBrains Mono",monospace',fontWeight:700,marginTop:4}}>{info.vetPackPrice.toLocaleString()} ECO · ×{(me.foodInventory||{})[packKey]||0} owned</div>
                </button>
              );
            })}
          </div>
...
/* ============================================================
   TRAVEL SCREEN — expeditions replace Space
   ============================================================ */
function TravelScreen({ me, users, onBack, onSendExpedition, onCollectExpedition, onToast }) {
  ...
  const destinations = [
    { id:'forest',     label:'FOREST',      emoji:'🌲', duration:EXPEDITION_DURATION_MS, materials:['wood','stone'],        desc:'Gather wood and stone' },
    { id:'quarry',     label:'QUARRY',       emoji:'⛏️', duration:EXPEDITION_DURATION_MS, materials:['stone','brick'],       desc:'Gather stone and brick' },
    { id:'desert',     label:'DESERT',       emoji:'🏜️', duration:EXPEDITION_DURATION_MS, materials:['glass','stone'],       desc:'Gather glass and stone' },
    { id:'jungle',     label:'JUNGLE',       emoji:'🌴', duration:EXPEDITION_DURATION_MS, materials:['wood','glass','brick'], desc:'Gather all materials' },
  ];
  ...
  return (
    <div style={{padding:'24px 28px 40px'}}>
      <div style={{marginBottom:18}}>
        ...
        <div style={{fontSize:12,color:'#a8a29e',marginTop:4}}>Send animals on 1-minute expeditions · earn greenhouse materials · risk of injury</div>
      </div>
      ...
      <div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:20}}>
        {Object.entries(MATERIAL_ICONS).map(([mat,icon])=>(
          <div key={mat} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 12px',borderRadius:8,background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)'}}>
            ...
            <span style={{fontFamily:'"JetBrains Mono",monospace',fontSize:12,fontWeight:700,color:'#4ade80'}}>×{(me.materials||{})[mat]||0}</span>
          </div>
        ))}
      </div>
            ...
            {destinations.map(d=>(
              <button key={d.id} onClick={()=>setSelDest(d.id)} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,border:selDest===d.id?'1px solid #4ade80':'1px solid rgba(255,255,255,0.08)',background:selDest===d.id?'rgba(74,222,128,0.12)':'rgba(255,255,255,0.03)',cursor:'pointer',textAlign:'left'}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:9,color:'#a8a29e',fontFamily:'"JetBrains Mono",monospace',marginTop:2}}>{d.desc} · 1 min · 25% injury risk</div>
                </div>
              </button>
            ))}
          </div>
            ...
            {availableCards.map(card=>{
              ...
              return (
                <button key={card.id} onClick={()=>setSelCard(card)} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,border:isSel?`1px solid ${me.color}`:'1px solid rgba(255,255,255,0.08)',background:isSel?`${me.color}15`:'rgba(255,255,255,0.03)',cursor:'pointer',textAlign:'left'}}>
                  <div style={{flex:1}}>
                    <div style={{fontFamily:'"Bebas Neue",sans-serif',fontSize:14,color:'#fff7ed',letterSpacing:'0.04em',lineHeight:1}}>{card.first} {card.last}</div>
                    <div style={{fontSize:9,color:'#a8a29e',fontFamily:'"JetBrains Mono",monospace',marginTop:2}}>{card.pps}/sec · {card.team}</div>
                  </div>
                </button>
              );
            })}
              ...
              return (
                <div key={exp.id} style={{padding:'14px 16px',borderRadius:12,background:'rgba(255,255,255,0.03)',border:`1px solid ${done?'#4ade80':'rgba(255,255,255,0.08)'}`}}>
                  <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:8}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontFamily:'"Bebas Neue",sans-serif',fontSize:15,color:'#fff7ed',letterSpacing:'0.04em'}}>{card.first} {card.last} → {destInfo.label}</div>
                      <div style={{fontSize:9,color:'#a8a29e',fontFamily:'"JetBrains Mono",monospace',marginTop:2}}>
                        {done ? '✅ READY TO COLLECT' : `${Math.ceil(remaining/1000)}s remaining`}
                      </div>
                    </div>
                    {done && (
...
/* ============================================================
   GREENHOUSE SCREEN — replaces Binders
   ============================================================ */
    ...
    <div style={{padding:'24px 28px 40px'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:18,flexWrap:'wrap',gap:12}}>
        <div>
          ...
          <div style={{fontSize:12,color:'#a8a29e',marginTop:4}}>Build greenhouses with materials from expeditions · house your animals inside</div>
        </div>
      ...
      <div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:20}}>
        {Object.entries(MATERIAL_ICONS).map(([mat,icon])=>(
          <div key={mat} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 12px',borderRadius:8,background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)'}}>
            <span style={{fontSize:16}}>{icon}</span>
            <span style={{fontFamily:'"Bebas Neue",sans-serif',fontSize:13,color:'#fff7ed',letterSpacing:'0.06em'}}>{mat.toUpperCase()}</span>
            <span style={{fontFamily:'"JetBrains Mono",monospace',fontSize:12,fontWeight:700,color:'#4ade80'}}>×{mats[mat]||0}</span>
          </div>
        ))}
      </div>
        ...
        {greenhouses.map(gh=>{
          ...
          return (
            <div key={gh.id} style={{padding:'16px',borderRadius:14,background:'rgba(74,222,128,0.06)',border:'1px solid rgba(74,222,128,0.25)',cursor:'pointer'}}>
              ...
              <div style={{fontSize:10,color:'#a8a29e',fontFamily:'"JetBrains Mono",monospace',marginTop:4}}>{animalCount}/{size?.slots||9} ANIMALS · {size?.label}</div>
...
SR(
  ...
  const handleFeedAnimal = (cardId, foodId) => {
    ...
    showToast('+' + pct + '% HUNGER · ' + card.first + ' ' + card.last);
  };
  ...
  const handleSendExpedition = (cardId) => {
    ...
    const card = me.ownedCards.find(c=>c.id===cardId);
    showToast((card?.first||'ANIMAL') + ' SENT ON EXPEDITION · 1 MIN');
  };
  ...
  const handleCollectExpedition = (expId) => {
    ...
    const matStr = Object.entries(earned).map(([m,n])=>n+'×'+m.toUpperCase()).join(' · ');
    if (injured && card) {
      showToast(card.first+' RETURNED INJURED · '+matStr, 'err');
    } else {
...
SR(
  `  // Damage accrual`,
  `  // Expedition completion check — poll every 5s
  useEffect(()=>{
    if(!me) return;
...
// ============================================================
// S7: Nav bar — replace dock with new eco nav
// ============================================================
SR(
...
// ============================================================
// S8: Mobile nav sheet — add new screens
// ============================================================
SR(
...
Write-Host "  FOOD screen  - feed animals, use vet packs, buy food + vet packs"
Write-Host "  TRAVEL screen - send animals on 1-min expeditions, collect materials"
Write-Host "  GREENHOUSE screen - build with materials, 5 sizes XS->XL"
Write-Host "  New nav: HOME COLLECT VET GREENHOUSE DESIGN SHOP TRAVEL INBOX FOOD LOG OUT"
Write-Host "  Injury system - 25% chance on expedition return"
Write-Host "  Material system - wood stone glass brick"
Write-Host ""
